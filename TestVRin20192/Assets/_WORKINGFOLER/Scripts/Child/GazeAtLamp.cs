...
public class GazeAtLamp : MonoBehaviour
{
    ...
    // sounds
    public GameObject onLampSound;
    public GameObject offLampSound;
    ...
    private IEnumerator WaitGazingTillFinish()
    {
        ...
        if (firstState)
        {
            onLampSound.SetActive(false);
            offLampSound.SetActive(true);
            animator.SetTrigger("TriggerSecondState");
            firstState = false;
        }
        else if (!firstState)
        {
            onLampSound.SetActive(true);
            offLampSound.SetActive(false);
